#!/bin/bash  -l 
# updated for uenv builds on santis/clariden/alps etc
...
if [ "$7" = "$MACHINE_NAME-5.12.0-NVIDIA" ]; then
  # paraview 5.12.0 nvidia EGL 
  SQUASH_IMG=@SQUASHFS_IMAGE_NAME@
  SQUASH_CMD="squashfs-mount $SQUASH_IMG:/user-environment"
  PV_SERVER=@PARAVIEW_BINARY_PATH@
  echo "path @PARAVIEW_BINARY_PATH@ from squashfs $SQUASH_IMG"
